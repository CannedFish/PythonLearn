...
class QuestionMethodTests(TestCase):
  ...
  def test_was_published_recently_with_future_question(self):
    '''
    was_published_resently() should return False for questions whose
  ...
  def test_was_published_recently_with_old_question(self):
    ...
    self.assertEqual(old_question.was_published_resently(), False);

  def test_was_published_recently_with_recent_question(self):
    '''
    was_published_resently() should return True for questions whose
    pub_date is within the last day.
    '''
    time = timezone.now() - datetime.timedelta(hours = 1);
    old_question = Question(pub_date = time);
    self.assertEqual(old_question.was_published_resently(), True);
...
class QuestionViewTests(TestCase):
  ...
  def test_index_view_with_a_future_questions(self):
    '''
    Questions with a pub_date in the future should be displayed on the
    index page.
    '''
...
class QuestionIndexDetailTests(TestCase):
  def test_detail_view_with_a_future_question(self):
    '''
    The datail view of a question with a pub_date in the future should
  ...
  def test_detail_view_with_a_past_question(self):
    '''
    The datail view of a question with a pub_date in the past should
